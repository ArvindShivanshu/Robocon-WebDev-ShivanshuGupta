...
import { TiltedCard, ShinyText, DecryptedText } from './reactbits';
...
export default function LivePassPreview({ formData }) {
  ...
  const accentColor = isSolidworks ? '#f97316' : isAltium ? '#06b6d4' : '#f59e0b';
    ...
    <div style={{ width: '100%', maxWidth: '380px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        ...
        <span style={{ fontSize: '0.72rem', color: '#10b981', fontWeight: 600 }}>
          Live Generated • 3D Gyro
        </span>
      </div>

      {/* React Bits TiltedCard for 3D Perspective & Specular Sheen */}
      <TiltedCard
        maxTilt={14}
        scale={1.03}
        perspective={850}
        glare={true}
        glareColor="rgba(226, 183, 104, 0.16)"
      >
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', paddingBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span className="font-mono" style={{ fontSize: '0.65rem', color: accentColor, fontWeight: 700 }}>
                <ShinyText speed={2.5}>VERIFIED // SEAT</ShinyText>
              </span>
            </div>
          </div>
          ...
          {/* ENIG Gold Contact Edge Fingers with Metallic Sheen */}
          <div
            style={{
              display: 'flex',
            ...
            {Array.from({ length: 22 }).map((_, i) => (
              <div
                ...
                style={{
                  width: '6px',
                  height: '16px',
                  background: 'linear-gradient(180deg, #fce09b 0%, #e2b768 70%, #997836 100%)',
                  borderRadius: '1px 1px 0 0',
                  opacity: 0.95,
                  boxShadow: '0 0 4px rgba(226, 183, 104, 0.45)'
                }}
              />
            ))}
          </div>
